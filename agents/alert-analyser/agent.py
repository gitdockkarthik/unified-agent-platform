...
class AgentRunner:
    ...
    The Anthropic API key is resolved at call time: if the backend passes
    X-Anthropic-Key, that key is used; otherwise falls back to
    ANTHROPIC_API_KEY from env (useful for direct agent invocation in dev).
    """
    ...
    async def run(
        ...
        """Invoke Claude, handling the tool-use loop. Returns (text, total_tokens).

        api_key: forwarded from the X-Anthropic-Key request header; falls back to
